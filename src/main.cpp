...
static const int64 nTargetTimespan = 24 * 60 * 60;  // 24-hour
static const int64 nTargetSpacingWorkMax = 2 * nStakeTargetSpacing; // ten minutes
...
unsigned char pchMessageStart[4] = { 0xe4, 0xec, 0xe6, 0xe5 };
...
void GenerateNoirShares(bool fGenerate, CWallet* pwallet)
{
    if (nThreads==0){
		...
		minerThreads->create_thread(boost::bind(&NoirSharesMiner, pwallet, rand()));
